...
public class BinarySearchTreeNode<T extends Comparable<T>> 
{
	...
	/*
	* Insert a new node based on if . or - as a descendant of this node * @param newNode the node to insert, start index of string, code
	*/
	public void addNode(BinarySearchTreeNode<T> newNode,int start,String code) {
		char startChar = code.charAt(start);
		
		if (startChar == '.') { // go left
			if (left != null) {
				left.addNode(newNode, start+1, code);
			} else {
				left = newNode;
			}
		} else if (startChar == '-') { // go right
			if (right != null) {
